...
import { Construction, Calendar, Ban } from 'lucide-react';
...
const initialFormData = {
  ...
  windShoreDetail: '', wave: '', warn: '', alert: '', visitors: '', feature: '',
  jpWarning: '', forWarning: '', note: '', handover: '', jpTourist: '', forTourist: '', carType: '', carNo: '',
  unpatrolled: false, area: '', beach: '', seq: 1
};

// ログ入力（新規登録）専用画面。
// デザイン・構造はEditView.jsx（ログ編集画面）に統一している（罫線区切りの2カラムグリッド、
// ラベル+入力を縦積みしたセル）。windShoreDetail（ビーチに対しての風向）もEditViewと同じ
// 実装で追加済み（必須項目対象外）。
...
useEffect(() => {
  ...
  // 必須項目の定義（EditView.jsxのisFormValidと統一。windShoreDetail（ビーチに対しての風向）は
  // ログ入力画面のデータ仕様に存在しないため対象外）
  ...
  // 配列は長さで判定する（マルチセレクトは空文字ではなく空配列で「未選択」になるため）
  const isValueFilled = (v) => Array.isArray(v) ? v.length > 0 : (v !== null && v !== undefined && v !== '');
  // Unpatrolled（パトロール未実施）時はnoteのみ必須
  ...
  const isValid = formData.unpatrolled
  ...
  // EditView.jsxと同じグリッド構造（左右ペアのCSS Grid・罫線区切り・グレー背景）で組む。
  ...
  const rows = [
    ...
    },
    {
      ...
      right: {
        label: 'ビーチに対しての風向',
        // windShoreDetailは必須項目対象外（任意項目）のためcellFilledではグレーにならない。
        // 優先度と同じく、unpatrolled時は白のまま・通常時は入力済みでグレーという挙動を
        // highlightColorで個別に再現する（EditView.jsxと同じ扱い）
    ...
    },
    {
      ...
      right: {
        label: '優先度',
        // priorityは必須項目対象外（任意項目）のためcellFilledではグレーにならない。
        // 他の必須項目セルと同じ「unpatrolled時は白のまま・通常時は入力済みでグレー」という
        // 挙動に揃えるため、同条件をhighlightColorで個別に再現する（EditView.jsxと同じ扱い）
...
// ヘッダー〜エリア/ビーチ/日付/保存ボタンをまとめてスクロール上部に固定する（EditView.jsxと同様）
...
// EditView.jsxと同じく、選択肢の文字数に関わらずflex:1で等幅にする
...
// ログ入力画面はカード枠なしのグリッド。白背景に罫線区切り（EditView.jsxと同じ構造）
